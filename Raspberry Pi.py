...
class CameraManager:
    ...
    def init_camera_auto(self):
        self.logger.info("🔍 v4l2 카메라 디바이스 자동 탐지 중...")
...
class SimpleIOUTracker:
    ...
    def __init__(self, max_lost=5, iou_threshold=0.3, movement_threshold=2):  # 이동 임계값 2로 낮춤
        self.tracks = {}
        self.next_id = 1
...
class TrafficLightColorDetector:
    ...
    def detect_traffic_light_color(self, roi):
        ...
        red_lower1 = np.array([0, 100, 100])  # 밝기 증가
        red_upper1 = np.array([10, 255, 255])
        red_lower2 = np.array([170, 100, 100])  # 밝기 증가
        red_upper2 = np.array([180, 255, 255])
        red_lower3 = np.array([0, 80, 80])  # 채도/밝기 하향
        red_upper3 = np.array([15, 255, 255])  # Hue 범위 확장
        red_lower4 = np.array([165, 80, 80])  # 채도/밝기 하향
        red_upper4 = np.array([180, 255, 255])

        yellow_lower = np.array([18, 120, 120])  # 범위 축소
        yellow_upper = np.array([35, 255, 255])

        green_lower = np.array([45, 100, 100])  # 범위 축소
    ...
    def filter_vehicles_by_lane(self, vehicle_detections, frame_width, frame_height, frame):
        ...
        for detection in vehicle_detections:
            ...
            if abs(vehicle_center_x - lane_center_x) < lane_width / 2 and w * h > min_area:
                detection['in_zone'] = True  # 존 내 차량 마킹
    ...
    def run_optimized(self):
        ...
        while self.running:
            try:
                ...
                if self.video_writer is None or (time.time() - segment_start_time) > self.config['recording'][
                    'segment_duration']:
                    if self.video_writer:
                        self.video_writer.release()
                ...
                if fps_counter % 30 == 0:
                    ...
                    self.logger.info(
                        f"📊 FPS: {fps:.1f}, 추적: {active_tracks}, 이동중: {moving_vehicles}, 신호등: {self.detection_stats['traffic_lights']}, 변화: {self.detection_stats['traffic_light_changes']}, 출발: {self.detection_stats['vehicle_departures']}")
