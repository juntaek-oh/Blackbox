...
class CameraManager:
    def __init__(self, logger, config=None):  # ✅ config 매개변수 추가
        self.logger = logger
        self.camera = None
        self.config = config or {}  # ✅ config 저장

    def init_camera_auto(self, device_id=0):
        if device_id is None:
            device_id = self.config.get('camera', {}).get('device_id', 0)

        self.logger.info(f"📷 윈도우: DirectShow webcam {device_id} 탐지 중...")

        # device_id가 "auto"인 경우 처리
        device_candidates = []
        try:
            if isinstance(device_id, str) and device_id.lower() == "auto":
                device_candidates = list(range(0, 6))
            else:
                device_candidates = [int(device_id)]
        except Exception:
            device_candidates = [0]  # 기본값

        backends = [cv2.CAP_DSHOW, cv2.CAP_MSMF, cv2.CAP_ANY]

        for did in device_candidates:
            for be in backends:
                self.logger.info(f"윈도우: webcam {did} 탐지 중... backend={be}")
                cap = cv2.VideoCapture(did, be)
                if cap.isOpened():
                    ret, frame = cap.read()
                    if ret and frame is not None and frame.size > 0:
                        h, w = frame.shape[:2]
                        ch = frame.shape[2] if len(frame.shape) == 3 else 1
                        self.logger.info(f"카메라 OK: id={did}, backend={be}, {w}x{h}x{ch}")
                        self.camera = cap
                        self.apply_camera_settings()
                        return True
                    cap.release()

        self.logger.error("❌ 카메라 장치 오픈 실패")
        return False
    ...
    def apply_camera_settings(self):
        try:
            ...
            buffer_size = self.config.get('camera', {}).get('buffer_size', 1)
            ...
            self.logger.warning(f"카메라 설정 적용 실패: {e}")
...
class LaneDetector:
    ...
    def detect_lanes(self, frame):
        ...
        roi_vertices = np.array([
            [(0, height), (width // 2 - 50, int(height * 0.65)),
             (width // 2 + 50, int(height * 0.65)), (width, height)]
        ], dtype=np.int32)
...
class SimpleIOUTracker:
    """IoU 기반 단순 객체 추적기 (차선 존 내 감도 향상)"""

    def __init__(self, max_lost=5, iou_threshold=0.3, movement_threshold=2):
        self.tracks = {}
        self.next_id = 1
...
class ProfessionalSmartBlackBox:
    """IoU 기반 차량 추적 블랙박스 시스템"""

    def __init__(self, config_path="blackbox_config.json"):  # ✅ 경로 단순화
        self.setup_logging()
        self.config_path = config_path
        self.config = self.load_config()

        # ✅ config를 CameraManager에 전달
        self.camera_manager = CameraManager(self.logger, self.config)
        self.net = None
        self.classes = []
        self.output_layers = []
        ...
        self.event_log_file = os.path.join("log", "event_log.txt")
        ...
        self.logger.info("✅ IoU 기반 차량 추적 블랙박스 시스템 초기화 완료")
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(
            ...
            handlers=[
                logging.FileHandler('log/blackbox.log', encoding='utf-8'),
    ...
    def load_config(self):
        default_config = {
            ...
            "recording": {
                "output_dir": "webcam_output",  # ✅ 상대 경로 사용
    ...
    def init_camera(self):
        device_id = self.config.get('camera', {}).get('device_id', 0)
        return self.camera_manager.init_camera_auto(device_id)
    ...
    def init_ai_model(self):
        try:
            ...
            if not all(os.path.exists(p) for p in [weights_path, config_path, classes_path]):
                self.logger.warning("AI 모델 파일이 없습니다. 기본 녹화 모드로 실행됩니다.")
                return False
    ...
    def create_video_writer(self, frame_shape):
        try:
            ...
            if writer.isOpened():
                self.logger.info(f"✅ 녹화 시작: {filename} ({width}x{height})")
                return writer, filepath
            else:
                self.logger.error(f"⚠ 비디오 라이터 생성 실패: {filepath}")
                return None, None
        except Exception as e:
    ...
    def _log_worker(self):
        ...
        while True:
            try:
                ...
                self.logger.error(f"비동기 로그 쓰기 실패: {e}")
    ...
    def log_vehicle_departure(self, track):
        if track.get('departure_detected', False):
            ...
            self.log_queue.put(log_message)
            self.logger.info(f"🚗 차량 출발: ID{track['id']}")
    ...
    def draw_lane_overlay(self, overlay, lanes, center_points, width, height):
        ...
        if center_points:
            ...
            for point in center_points:
                cx, cy = point
                cv2.rectangle(overlay, (cx - lane_width // 2, cy - 2), (cx + lane_width // 2, cy + 2), (0, 255, 255),
                              -1)
    ...
    def draw_optimized_overlay(self, frame, detections, tracked_vehicles):
        ...
        # 상단 패널 축소
        panel_height = 60  # ✅ 140 → 60으로 축소
        cv2.rectangle(overlay, (0, 0), (overlay.shape[1], panel_height), (0, 0, 0), -1)

        timestamp = datetime.now().strftime("%H:%M:%S")
        cv2.putText(overlay, f"TIME: {timestamp}", (10, 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        cv2.putText(overlay, "LANE-BASED FRONT VEHICLE TRACKING", (10, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)

        active_tracks = len(tracked_vehicles)
        moving_tracks = len([t for t in tracked_vehicles if t['is_moving']])
        stats_text = f"Tracks:{active_tracks} Moving:{moving_tracks} TL:{self.detection_stats['traffic_lights']} TC:{self.detection_stats['traffic_light_changes']} Dep:{self.detection_stats['vehicle_departures']}"
        cv2.putText(overlay, stats_text, (300, 20),
        ...
        if self.traffic_light_detector.last_stable_color:
            ...
            cv2.putText(overlay, light_status, (overlay.shape[1] - 200, 20),
            ...
            if total_size > max_size_bytes:
                ...
                for filepath, size, mtime in files:
                    try:
                        os.remove(filepath)
                        total_size -= size
                        self.logger.info(f"오래된 파일 삭제: {os.path.basename(filepath)}")
                        if total_size <= max_size_bytes * 0.8:
                            break
                    except Exception as e:
                        self.logger.error(f"파일 삭제 실패 {filepath}: {e}")
        except Exception as e:
    ...
    def cleanup_resources(self):
        ...
        if hasattr(self, 'log_buffer') and self.log_buffer:
            try:
                ...
                self.logger.error(f"로그 플러시 실패: {e}")
    ...
    def run_optimized(self):
        ...
        if not self.init_ai_model():
            self.logger.warning("AI 모델 없이 기본 녹화 모드로 실행")
        ...
        # 미리보기 창 설정
        cv2.namedWindow("BlackBox Live Preview", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("BlackBox Live Preview", 1280, 720)
            ...
            try:
                ...
                if not ret or frame is None:
                    ...
                    if self.frame_read_failures > self.max_frame_read_failures:
                        self.logger.error("연속 프레임 읽기 실패, 종료")
                        break
                ...
                if frame_count == 0:
                    h, w = frame.shape[:2]
                    self.logger.info(f"실제 카메라 해상도: {w}x{h}")

                # ✅ 세그먼트 녹화 처리 수정
                if (self.video_writer is None or
                        (time.time() - segment_start_time) > self.config['recording']['segment_duration']):
                ...
                # 실시간 미리보기 창 표시
                cv2.imshow("BlackBox Live Preview", overlay_frame)
                ...
                if fps_counter % 30 == 0:
                    ...
                    self.logger.info(
                        f"📊 FPS: {fps:.1f}, 추적: {active_tracks}, 이동중: {moving_vehicles}, "
                        f"신호등: {self.detection_stats['traffic_lights']}, 변화: {self.detection_stats['traffic_light_changes']}, "
                        f"출발: {self.detection_stats['vehicle_departures']}"
                    )
...
def main():
    parser = argparse.ArgumentParser(description='IoU 기반 차량 추적 블랙박스 (차선 기반 동적 앞차 감지 + TTS 최적화)')
    parser.add_argument('--config', default='blackbox_config.json', help='설정 파일 경로')
